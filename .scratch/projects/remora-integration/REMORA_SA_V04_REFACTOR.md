...
**Option A: Reimplement `load_manifest` in Remora**

The `load_manifest` function loaded YAML bundle manifests. Since Remora already has
YAML loading infrastructure, this should be reimplemented locally:

```python
# In remora/core/manifest.py (new file) or inline in swarm_executor.py
...
class BundleManifest(BaseModel):
    ...
    def load(cls, path: Path) -> "BundleManifest":
        """Load manifest from bundle path."""
        if path.is_dir():
...
**Option B: Keep dependency on agent.py**

If there's complex manifest logic you need, you could vendor the original `agent.py`
into Remora. However, Option A is recommended for cleaner separation.
